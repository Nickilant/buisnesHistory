...
- `POST /documents/availability` — неблокирующая проверка наличия PDF в КАД для кнопки документа; API кэширует результат на 1 час, проверяет только видимые документы и отключает кнопку только при уверенном ответе `unavailable`; неоднозначные ответы КАД оставляют ссылку кликабельной.
